...
async function startRecording() {
    ...
    try {
        ...
        // Create local reference — don't rely on module-level var in onstop
        ...
        recorder.onstop = async () => {
            ...
            // Use recorder.mimeType (local ref) not mediaRecorder (may be reassigned)
            ...
            if (blob.size < 500) {
                setStatus("Too short — hold longer and speak", "text-yellow-400")
                setTimeout(() => setStatus("Listening... hold button to speak", "text-green-400"), 2000)
                return
            }
...
function stopRecording() {
    ...
    // Stop recorder — onstop fires async, don't null mediaRecorder here
    mediaRecorder.stop()
    isRecording = false
